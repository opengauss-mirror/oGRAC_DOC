# SQL能力介绍<a name="ZH-CN_CONCEPT_0289895656"></a>
<!-- TOC -->
...
- 支持的SQL标准
    ...
    SQL语言是由美国国家标准化协会和国际标准化组织共同定义的一种标准化语言，行业中最主要的标准为SQL92和SQL99。2023年，发布了ISO/IEC 9075:2023SQL标准。从表的定义、数据类型、各类查询功能等不断演化。SQL标准由十个部分组成，其中一部分（SQL/RPR:2012）是2012年新增的，另外五个部分在2011年进行了修订，其余四个部分仍然沿用2008年的版本。
...
- 支持的开发接口
    ...
    oGRAC提供业界标准的ODBC及JDBC接口，保证用户业务快速迁移至oGRAC。
...
![SQL处理流程](./sql_process0.svg "SQL处理流程")
- SQL解析
  ...
  - oGRAC支持SQL Cache的功能，能够将用户输出的SQL语句与计划缓存中的副本进行比对，如果缓存中已经存在相同语句且不包含失效的数据库对象，则直接使用缓存中的计划，跳过计划生成的过程。
- SQL优化
